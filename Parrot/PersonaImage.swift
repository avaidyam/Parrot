...
public func imageForString(forString source: String, size: NSSize = NSSize(width: 64, height: 64), colors: [NSColor] = materialColors) -> NSImage {
	return NSImage(size: size, flipped: false) { rect in
		let c = colors[abs(source.hashValue) % colors.count]
        NSGradient(starting: c, ending: c.blended(withFraction: 0.33, of: .black)!)?.draw(in: rect, angle: 270)
		
        let textStyle = NSMutableParagraphStyle.default.mutableCopy() as! NSMutableParagraphStyle
		textStyle.alignment = .center
        let textSize = rect.size.width * 0.75
		let font = NSFont(name: ".SFCompactRounded-Medium", size: textSize) ??
                   NSFont.systemFont(ofSize: textSize, weight: .semibold)
		var rect2 = rect
		rect2.origin.y = rect.midY - (font.capHeight / 2)
        
        String(source.characters.first!).uppercased().draw(with: rect2, attributes: [
            .font: font,
...
public func defaultImageForString(forString source: String, size: NSSize = NSSize(width: 64, height: 64), colors: [NSColor] = materialColors) -> NSImage {
	return NSImage(size: size, flipped: false) { rect in
        ...
        NSImage(named: .user)!.draw(in: r, from: .zero, operation: .XOR, fraction: 1.0)
		return true
	}
}
