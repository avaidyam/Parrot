...
extension NSTableView {
    ...
    open func update<T>(with edits: [Edit<T>], in section: Int = 0) {
        ...
        self.beginUpdates()
        if !indexPaths.deletions.isEmpty { self.removeRows(at: IndexSet(indexPaths.deletions), withAnimation: [.effectFade, .slideDown]) }
        if !indexPaths.insertions.isEmpty { self.insertRows(at: IndexSet(indexPaths.insertions), withAnimation: [.effectFade, .slideUp]) }
